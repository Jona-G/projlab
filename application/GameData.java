...
public class GameData implements Serializable {
	...
	public GameData(GameData g) {
		repairmanScore = g.repairmanScore;
		saboteurScore = g.saboteurScore;
		fieldList = g.fieldList;
		playerList = g.playerList;
		random = g.random;
		randomDisabled = g.randomDisabled;
		
		fieldViewList = g.fieldViewList;
		playerViewList = g.playerViewList;
		infoTextArea  = g.infoTextArea;
		canvas = g.canvas;
		bottomPanel = g.bottomPanel;
		game = g.game;
		fieldMap = g.fieldMap;
		playerMap = g.playerMap;
		jatekosKovetkezik = g.jatekosKovetkezik;
		state = g.state;
	}
